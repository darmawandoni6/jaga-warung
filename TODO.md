...
| 2 | 2026-09-26 | Keyboard Fix | Android keyboard covering form fields: KAV `behavior` on Android changed from `undefined` (no-op) to `"padding"` in 5 forms (ProductForm, DebtForm, RestockModal, StockAdjustmentModal, CategoryManagementScreen) + explicit `android.softwareKeyboardLayoutMode: "resize"` in app.json. Verified: `yarn typecheck`, `yarn lint`, `yarn format:check` = 0 errors. Manual device test pending. | ✅ Done |
| 3 | 2026-09-26 | Barcode Validation | ProductForm: pre-check `getProductByBarcode` before insert/update (friendly alert with owning product name), generic save-error alert now surfaces `error.message`. UNIQUE constraint kept as safety net. Verified: `yarn typecheck`, `yarn lint`, `yarn format:check` = 0 errors. | ✅ Done |
| 4 | 2026-09-26 | Barcode Primary Key | DB Migration v8: barcode as PRIMARY KEY in products, drop id column. Existing empty/null barcodes filled with unique random barcodes (`899...`). transaction_items & stock_movements refactored to product_barcode. ProductForm barcode is required + added '🎲 Acak' generator button. All repositories, stores, and screens updated. Verified: `yarn typecheck` = 0 errors, `yarn lint` = 0 errors. | ✅ Done |
| 5 | 2026-09-26 | Barcode Generator Removal | ProductForm: removed random barcode generator button ('🎲 Acak') + `Dices` icon import, removed `generateRandomBarcode` function, removed `console.error(error)` in catch block (silent error handling). Committed d31ec9e. | ✅ Done |
